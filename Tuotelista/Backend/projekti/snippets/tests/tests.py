...
def test_view_put(tuote_factory):
    ...
    payload = {
        ...
        "hinta": 999999444.0,
        "kuvaus": "Muokattu.",
        "tuotekuva": SimpleUploadedFile(name='puhelin.jpg', content=open("./snippets/tests/puhelin.jpg", 'rb').read(), content_type='image/jpeg')
    }

    request = factory.put("/snippets/1", payload)
...
def test_view_del(tuote_factory):
    tuote_factory()
    request = factory.delete("/snippets/1")
    response = TuoteDetail.as_view()(request, pk=1)
